...
const MotionFeatureCard = ({ icon: Icon, title, description, delay }) => (
  ...
  >
    ...
    <h3 className="text-2xl font-semibold mb-3 text-green-800 font-poppins">
      {title}
    </h3>
...
const MotionHowItWorksStep = ({ step, title, description, imageUrl, side, aosDelay }) => (
  <motion.div
    ...
    } items-center my-24 space-y-10 md:space-y-0`}
  ...
  >
    ...
    <div className="w-full md:w-1/2 relative group">
      ...
      >
        {/* Decorative overlay with faint farm icon */}
        <div className="absolute inset-0 bg-gradient-to-t from-black/20 via-transparent to-green-100/10"></div>
        <div className="absolute bottom-6 right-6 opacity-15 text-white text-6xl">
          🌾
        </div>
      </div>
    </div>
...
<div className="relative h-screen overflow-hidden">
  ...
  >
    ...
    >
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid lg:grid-cols-4 gap-10">
            <MotionFeatureCard
              icon={Leaf}
              title="Hyper-Local"
              description="Fresh, local sourcing within 50 miles — reducing footprint and maximizing flavor."
              delay={200}
            />
            <MotionFeatureCard
              icon={CheckCircle}
              title="Blockchain Trust"
              description="Every crop tracked from soil to shelf — verifiable and transparent."
              delay={400}
            />
            <MotionFeatureCard
              icon={Users}
              title="Fair Profit Share"
              description="Farmers earn up to 80% — empowering rural economies sustainably."
              delay={600}
            />
            <MotionFeatureCard
              icon={Globe}
              title="Zero Waste Vision"
              description="AI-driven logistics redirect surplus to community kitchens and food banks."
              delay={800}
            />
          </div>
        </div>
      ...
      {/* WHY CHOOSE US */}
      <section
        className="py-40 relative bg-center text-white"
        style={{
          backgroundImage:
            "url('https://images.unsplash.com/photo-1556761175-5973dc0f32e7?auto=format&fit=crop&w=2100&q=80')",
          backgroundAttachment: "fixed",
      ...
      >
        ...
        <div className="relative z-10 max-w-5xl mx-auto text-center px-6">
          <motion.h2
            className="text-6xl font-extrabold mb-6 font-playfair drop-shadow-xl"
            initial={{ opacity: 0, scale: 0.8 }}
            whileInView={{ opacity: 1, scale: 1 }}
          ...
          <motion.p
            className="text-2xl mb-16 font-light max-w-3xl mx-auto"
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
      ...
      <section className="py-32 bg-white relative z-20">
        <div className="max-w-7xl mx-auto px-6">
          <motion.h2
            className="text-5xl font-extrabold text-center mb-20 text-green-800 font-playfair"
            initial={{ opacity: 0, y: -20 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
          >
